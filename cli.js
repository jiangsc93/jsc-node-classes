...
const paint = (raw, colors) => {
    ...
    for (const color of colors) {
        start = chalk__default["default"][color](start);
    }
    return start;
};
class Printer {
...
const parseArguments = (commander, parsed) => {
    ...
    return commanderArgs.reduce((acc, cur, idx) => {
        ...
        acc[string.stringCamelCase(argument)] = args[idx];
        return acc;
    }, {});
};
const parseOptions = (commander, parsed) => {
    const options2 = {};
    object.objectEach(commander.options, (option, key) => {
        if (!object.objectHas(parsed, key))
            return;
        // const { type, default: dft, message } = option;
        // eslint-disable-next-line @typescript-eslint/no-unsafe-assignment
        options2[key] = parsed[key];
        // eslint-disable-next-line @typescript-eslint/no-unsafe-assignment
        options2[string.stringCamelCase(key)] = parsed[key];
    });
...
class Cli extends Events__default["default"] {
    constructor(options) {
        super();
        ...
        this._version = '';
        this._banner = '';
        this._options = object.objectAssign({}, defaults, options);
    ...
    printHelp(command) {
        ...
        object.objectEach(commander.options, (val, key) => {
    ...
    parse(argv = process.argv) {
        ...
        // 一定是全局命令
        if (inputLength === 0) {
            if (type.isString(longVersion) && parsed[longVersion])
                this.printVersion();
            else if (type.isString(longHelp) && parsed[longHelp])
                this.printHelp();
            else
        ...
        // 可能是局部命令（优先匹配）、全局命令
            ...
            if (type.isString(longHelp) && parsed[longHelp]) {
    ...
    async _execCommanderAction(commander) {
        ...
        const boolean = [];
        const defaults = {};
        object.objectEach(options, (option, key) => {
        ...
        try {
            ...
            const stdErr = error.errorNormalize(err);
            if (debug) {
...
Object.defineProperty(exports, 'minimist', {
    enumerable: true,
    get: function () { return minimist__default["default"]; }
});
