...
type TaskList = (Task | undefined)[];
interface RunnerStep {
    index: number;
    label: string;
    taskList: TaskList;
    beforeHookList: TaskList;
    afterHookList: TaskList;
    status: RunnerStatus;
}

export default class Runner extends Events {
    private _stepMap: Map<string, RunnerStep>;
    private readonly _stepList: RunnerStep[];
    private _logger: Logger;
    private _stashSuccessMessageList: string[];
    private _stashWarningMessageList: string[];
    private _status: RunnerStatus;

    constructor() {
        super();
        this._stepMap = new Map<string, RunnerStep>();
        this._stepList = [];
     ...
     * @returns {this}
     */
    step(label: string, ...taskList: TaskList): this {
     ...
     * @returns {this}
     */
    before(label: string, ...hookList: TaskList): this {
        if (this._status === RunnerStatus.stoped) {
     ...
     * @returns {this}
     */
    after(label: string, ...hookList: TaskList): this {
        if (this._status === RunnerStatus.stoped) {
    ...
    async _callHook(hookList: TaskList, input?: any): Promise<any> {
     ...
     * @returns {this}
     */
    stashSuccess(message: string): this {
     ...
     * @returns {this}
     */
    stashWarning(message: string): this {
     ...
     * @returns {Promise<any>}
     */
    async start<R = unknown>(input?: any): Promise<R> {
